...
server.init = function() {
  ...
  server.httpServer.listen(config.httpPort, function() {
    console.log('\x1b[35m%s\x1b[0m', `The server is listening HTTP on port ${config.httpPort} now (${config.envName} environment)`);
  });
  ...
  server.httpsServer.listen(config.httpsPort, function() {
    console.log('\x1b[36m%s\x1b[0m', `The server is listening HTTPS on port ${config.httpsPort} now (${config.envName} environment)`);
  })
};
