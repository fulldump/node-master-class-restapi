  ...
  return Promise.all([
    new Promise(function(resolve, reject) {
      ...
      that.httpServer.close(function() {
        console.log('\x1b[35m%s\x1b[0m', `HTTP server stopped`);
    ...
    new Promise(function(resolve, reject) {
      ...
      that.httpsServer.close(function() {
        console.log('\x1b[35m%s\x1b[0m', `HTTPS server stopped`);
...
Server.prototype.mainHandler = function(req, res) {
  ...
  // Prepare empty parameters object
  var parameters = {};
  ...
  const handler = this.router.match(method, path, parameters);
    ...
    handler(data, function(statusCode, payload) {
      ...
      if (payload) {
        ...
        for (var k in payload) {
          // All fields starting by `__` are considered private and will not be
          // exported by this serializer
          if (k.startsWith('__')) {
            delete payload[k];
          }
        }
